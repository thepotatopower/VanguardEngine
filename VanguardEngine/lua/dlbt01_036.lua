...
function GetParam(n)
	if n == 1 then
		return q.Location, l.Looking, q.NameContains, "Earnescorrect", q.Grade, 2, q.Other, o.GradeOrLess, q.Count, 1, q.Min, 0
	elseif n == 2 then
...
function ActivationRequirement(n)
	if n == 1 then
		return a.PlacedOnVC, p.HasPrompt, p.SB, 1
	elseif n == 2 then
...
function CheckCondition(n)
	if n == 1 then
		if obj.PlacedByRidingOn("Serious Challenger, Clarissa") then 
			return true
		end
	elseif n == 2 then
